...
- **Vulkan（MinGW，`situation_view`）**：Khronos 头文件为 **`third_party/Vulkan-Headers` 子模块**（`git submodule update --init`）。**不向 git 提交** MinGW 导入库 `libvulkan-1.dll.a`；仓库仅保留文本 **`third_party/vulkan-win32-mingw/vulkan-1.def`**（见 `.gitignore`）。克隆后在本机生成导入库：PowerShell **`powershell -NoProfile -ExecutionPolicy Bypass -File scripts/bootstrap_vulkan_mingw.ps1`**，或 MSYS2/Git Bash 下 **`bash scripts/bootstrap_vulkan_mingw.sh`**。需 **`dlltool` 在 PATH**，或设置环境变量 **`MINGW_DLLTOOL`** 指向 `dlltool.exe` 绝对路径。根目录 **`scripts/build.ps1`** 在 Ninja 前若发现缺少 `libvulkan-1.dll.a` 会自动运行上述 PowerShell 脚本。纯命令行构建时请先手动执行 bootstrap 再 `ninja -f build.ninja`。运行时仍依赖系统 **`vulkan-1.dll`**（一般由显卡驱动提供）。
