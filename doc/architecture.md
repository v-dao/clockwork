...
引擎负责**调度顺序**与**跨实体交互**（同一仿真步内的消息与探测一致性）。当前实现中，每步先按 `comdevice` → `processor` → `weapon` → `signature` → `mover` 调度，再在态势聚合末尾执行 **sensor**探测（`run_model_pass(Sensor)`），以便使用本帧更新后的位姿写入 `snapshot_.sensor_detections`。实体较多时，探测在 **Web 墨卡托平面 (x,y)** 上使用**均匀网格 / 空间哈希**做粗筛（按本步全局最大传感器射程定格宽），再对候选做距离与 FOV 精判；小规模仍用直接两两枚举。
