...
### comdevice（缓存占位，阶段 6 扩展）


| 键 | 类型   | 说明                                      |
| --------- | ---- | --------------------------------------- |
| `node_id` | 字符串 | 可选，与想定 `comm_node` 的 id 对齐，写入 `EntityComdeviceCache` |
...
| 键 | 类型 | 说明                                                         |
| ---------- | -- | ---------------------------------------------------------- |
| `rounds`   | 浮点 | 非负，截断为整数后写入 `EntityWeaponCache::rounds_ready`；非法或缺省为 0 |
| `magazine` | 浮点 | 若 `rounds` 未给出（为 0）且本键非负，则用其截断为整数作为待发计数 |
