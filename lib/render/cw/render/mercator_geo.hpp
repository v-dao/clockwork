...
inline void mercator_meters_to_lonlat(double x, double y, double& lon_deg, double& lat_deg) {
  ...
  const double lat = 2.0 * std::atan(std::exp(yc / kR)) - kPi * 0.5;
  lon_deg = lon * (180.0 / kPi);
  lat_deg = lat * (180.0 / kPi);
}
...
inline void lonlat_deg_to_mercator_meters(double lon_deg, double lat_deg, double& mx, double& my) {
  ...
  const double lonr = lon_wrapped * (kPi / 180.0);
  const double latr = lat_deg * (kPi / 180.0);
  mx = wrap_mercator_lon_x(kR * lonr);
  const double y_raw = kR * std::log(std::tan(kPi * 0.25 + latr * 0.5));
